...
- **便携版自动更新重试与真·无窗口执行（修复独占锁未释放导致更新未覆盖及终端弹窗）**：
  1. **文件锁重试循环**：批处理引入 `:copyloop` 重试循环（最多 20 次，每次 500ms），等待旧版进程完全退出释放 Windows 内核文件独占锁后再覆盖，彻底解决单次复制卡锁失败；
  2. **彻底解决黑框终端弹窗**：改用 Windows 原生 GUI 子系统的 `WScript.Shell.Run`（`0` 隐藏模式）拉起批处理，避免底层 `CREATE_NO_WINDOW` 与 `DETACHED_PROCESS` 互斥触发控制台窗口；
  3. **退出即刻释放锁**：调用更新脚本后主进程立即强制退出，加速释放 `.exe` 文件句柄。
