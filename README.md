...
- **导入 PDF / Word 文档底图**：任意 PDF（pypdfium2 栅格化）或 DOCX（Word COM / LibreOffice 转 PDF）逐页渲染为多页背景，直接在「打印稿」上框选手写填写，适合表格、试卷、实验报告等场景；文档中的高亮色块或 `{{...}}`、`【...】` 占位标签会被自动检测——选「提取填空框」则擦除高亮底色、自动生成手写填空区域并按高亮颜色关联笔迹角色，选「保留完整底图」则原样保留色彩不生成填空（适合自带色块 / 表格样式的完整模板）
