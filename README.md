...
- **导入 docx**：自动解析段落对齐方式与首行缩进（支持 Word 的「首行缩进 2 字符」写法，沿样式链继承）；若文档带有高亮 / 背景色，导入时弹窗选择排版方式——「全部作为手写」忽略高亮整篇手写，「打印/手写混排」则高亮文字用手写体、未高亮文字保留打印体
- **划选标记与笔迹角色**：正文编辑器中划选文字后一键「设为打印 / 角色1 黄 / 角色2 绿 / 清除标记」，以背景色直观预览；「笔迹角色管理」为每个角色单独配置字体 / 颜色 / 扰动 σ，打印体角色强制零扰动零错字；Word 中首次出现的背景高亮自动映射为手写角色1，次出现映射为手写角色2……
...
- **写错字模拟**：错字率 0~30% 滑杆调节，错字划掉后按「右上方重写」（正上方略偏右、小一号、同一支笔加粗）或「后文重写」两种方式补写，涂改样式支持单线 / 双线 / 斜线 / 叉号
...
- **导入 PDF / Word 文档底图**：任意 PDF（pypdfium2 栅格化）或 DOCX（Word COM / LibreOffice 转 PDF）逐页渲染为多页背景，直接在「打印稿」上框选手写填写，适合表格、试卷、实验报告等场景；**强烈推荐先在 Word 中另存为 PDF 再导入**，版式完全固化、像素级还原，避免 Word 直接导入受本机字体/渲染差异导致的表格错位；文档中的高亮色块或 `{{...}}`、`【...】` 占位标签会被自动检测——选「提取填空框」则擦除高亮底色、自动生成手写填空区域并按高亮颜色关联笔迹角色，选「保留完整底图」则原样保留色彩不生成填空（适合自带色块 / 表格样式的完整模板）
...
- **一键导出**：全部页面导出为 `0.png`、`1.png`……到所选目录（默认 `output/`），或一键导出 **300 DPI 位图层 PDF**（img2pdf，PNG 无损嵌入）
...
> 也可以直接在 GitHub Actions 中构建：推送 `v*` 标签或手动触发 `Build and Release` workflow（`.github/workflows/build.yml`），会自动在 Windows / Linux / macOS 三平台打包并组装便携 zip + 单文件升级包（exe + 预设 + 背景 + fonts 目录），打标签时自动发布到 Release。

- 产物：`dist/HandWriteSim`（**单文件**，Windows 约 55 MB，Linux/macOS 体积相近）；Release 中每个平台另有**便携 zip**（首次使用，解压即用）与**单文件升级包**（老用户直接替换旧可执行文件即可，应用内自动更新用的就是这个文件）
...
**导入时弹出的「检测到高亮」对话框怎么选？**
- 导入 docx 到正文：选「全部作为手写」则忽略高亮整篇手写；选「打印/手写混排」则高亮文字用手写体、未高亮文字保留打印体。
- 导入 PDF / Word 文档底图：选「提取填空框」会擦除高亮底色、自动生成手写填空区域并按颜色关联笔迹角色；选「保留完整底图」则不擦除、不生成填空区域，适合带色块 / 表格样式的完整模板。

**软件里能查看功能使用说明吗？**
可以。主界面点「ℹ️ 关于」→「📖 使用指南」，分章节介绍了快速上手、基础排版、docx 智能混排、文档底图与填空、错字模拟等内容。
...
### 1. 文档制作与高亮标记
1. 在 Word（或 WPS）中打开文档；
2. 题目、表格、题干等保持**无背景色**（将原样保留为打印体底图）；
3. 选中需要手写填空的内容，用 Word 的**「突出显示文本颜色（高亮）」**标记背景色（如黄、绿、青等），或直接写 `{{...}}`、`【...】` 占位标签。

### 2. 导出 PDF 并导入（推荐）
- 在 Word 中「文件」→「另存为 PDF」；
- 点「导入文档底图」选择该 PDF；
- **为什么推荐 PDF？** Word 直接导入依赖本机 Word（COM，仅 Windows）或 LibreOffice，受字体/渲染差异影响易错位；转 PDF 后版式固化，逐页像素级还原。

### 3. 智能自动识别
- 逐页渲染为高清底图，**自动擦除高亮底色**恢复干净纸面，并抹除 `{{...}}` / `【...】` 标签文字；
- 按全角字符紧包围盒校准原文字号、行距、缩进，在对应位置自动生成手写区域；
- 不同高亮颜色自动归类为「手写角色 1」「手写角色 2」，可在「笔迹角色管理」中换字体/墨色。

### 4. 选框微调
- 右侧「文字区域」列表单击某项，或直接单击预览图上的区域，即可拖动整体平移、拖 8 向手柄缩放；
- 双击区域重新打开编辑属性。

### 5. 特殊需求
- **想保留背景色（不擦除、不生成填空）**：导入底图时选**「保留完整底图」**，原样保留色彩与文字，直接当普通背景。
- **模板带高亮但想通篇全手写**：用「导入 docx」到正文时选**「全部作为手写」**，忽略全部高亮标记，无需手动清格式。
